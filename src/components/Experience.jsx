      ...
      <div id="experience" className='relative py-20 '>
        ...
        <div ref={containerRef} className=' relative w-full h-full flex flex-col items-center justify-center gap-y-10 lg:gap-y-20 py-10'>
          {experienceData.map((data, i) => (
              <div 
                key={`id-${i}`} 
                className={`w-[600px] xl:w-[480px] sm:w-full px-12 sm:px-0 relative ${i % 2 === 0 ? "-left-[300px] xl:-left-[240px] sm:left-0" : "left-[300px] xl:left-[240px] sm:left-0"}`}
              >
                <motion.div  initial={{opacity: 0, x: i % 2 === 0 ? -80 : 80}} 
                  whileInView={{opacity: 1, x: 0}} 
                  viewport={{once: true}} 
                  transition={{duration: 0.9, type: 'spring', stiffness: 50}}
                  className="relative flex flex-col gap-y-3 rounded-md border border-red-300 bg-white p-4 tracking-wide sm:text-sm dark:bg-zinc-700  dark:text-white d transition-colors z-20 "
                >
          ...
          <motion.div initial={{ scaleY: 0 }} style={{ scaleY: scrollY }} className="absolute w-1 h-full rounded-full bg-gray-300 origin-top">
          </motion.div>
        </div>
      </div>
    )
